...
## Purpose
FlexyField integrates dynamic fields with Laravel's Eloquent query builder, enabling filtering, ordering, and searching on flexy fields as if they were native database columns. With attribute sets, queries can filter by attribute set context.

## MODIFIED Requirements

### Requirement: Query Scope Integration
The system SHALL provide automatic query scopes for flexy fields with attribute set context.
...
#### Scenario: Queries include attribute set context
- **WHEN** a query is executed on a model with flexy fields
- **THEN** results SHALL include models regardless of attribute_set_code
- **AND** flexy field values SHALL be scoped to each instance's attribute set
...
- **AND** models from different attribute sets with the same field SHALL be included
...
- **AND** ordering SHALL work across different attribute sets
...
### Requirement: Attribute Set Filtering
The system SHALL allow querying models by their assigned attribute set.

#### Scenario: Filter by attribute set code
- **WHEN** whereAttributeSet('set_code') is called
- **THEN** the query SHALL filter by attribute_set_code column
- **AND** only models assigned to the specified set SHALL be returned

#### Scenario: Filter by multiple attribute sets
- **WHEN** whereAttributeSetIn(['set1', 'set2']) is called
- **THEN** the query SHALL filter by attribute_set_code using IN clause
- **AND** models assigned to any of the specified sets SHALL be returned

#### Scenario: Query models without attribute set
- **WHEN** whereAttributeSetNull() is called
- **THEN** the query SHALL filter for null attribute_set_code
- **AND** only unassigned models SHALL be returned

### Requirement: Cross-Attribute-Set Queries
The system SHALL handle queries that span multiple attribute sets.

#### Scenario: Query field present in multiple sets
- **WHEN** where('flexy_color', 'red') is called
- **AND** 'color' field exists in multiple attribute sets
- **THEN** the query SHALL return models from all sets containing the field
- **AND** models SHALL match regardless of their attribute_set_code
...
### Requirement: Eager Loading with Attribute Sets
The system SHALL support efficient eager loading of flexy fields with attribute set context.
...
#### Scenario: Attribute set definitions are eager loadable
- **WHEN** with('attributeSet') is called on a model query
- **THEN** the AttributeSet relation SHALL be eager loaded
- **AND** the attribute set's fields SHALL be available
